...
def next():
    ...
    if count == 2:
        ...
        if before_or_after == 0:
            for i in range(1,6):
                before_powder.append(db.reference(member_db + usernumber + "/bp_" + str(i)).get())
                pow_number = 0
            ...
            # 자판기 안에 있음
            if check == 1:
                gram = db.reference(member_db + usernumber + "/bp_" + check_number + "gram").get()
                label_powtext_1.configure(text=powtext[pow_number])
            # 자판기 안에 없음 비슷한거 찾음
            else:
                required_gram = int(db.reference(member_db + usernumber + "/bp_" + check_number + "gram").get())
                amount_gram = int(db.reference(booster_db + before_powder[check_number] + "/amount").get())
                booster_protein = int(db.reference(booster_db + before_powder[1] + "/protein").get())
                required_protein = required_gram / amount_gram * booster_protein
                required_sort = before_powder[1][0:3]
                for i in range(1,7):
                    if vender_powder[i][0:3] == required_sort:
                        pow_number = i
                        break
                gram = required_protein / vender_prot[pow_number] * vender_amount[pow_number]
                label_powtext_1.configure(text=powtext[0] + powtext[pow_number])
        else:
            for i in range(1,6):
                after_powder[i] = db.reference(member_db + usernumber + "/ap_" + str(i))
                pow_number = 0
                for powders in vender_powder:
                ...
                if check == 1:
                    break
            if check == 1:
                gram = db.reference(member_db + usernumber + "/ap_" + check_number + "gram").get()
                label_powtext_1.configure(text=powtext[pow_number])
            else:
                gram = db.reference(member_db + usernumber + "/ap_" + check_number + "gram" )
                recommend_booster = after_powder[check_number]
                label_powtext_1.configure(text=powtext[pow_number])

                required_gram = int(db.reference(member_db + usernumber + "/ap_" + check_number + "gram").get())
                amount_gram = int(db.reference(booster_db + after_powder[check_number] + "/amount").get())
                booster_protein = int(db.reference(booster_db + after_powder[1] + "/protein").get())
                required_protein = required_gram / amount_gram * booster_protein
                required_sort = after_powder[1][0:3]
                for i in range(1, 7):
                    if vender_powder[i][0:3] == required_sort:
                        pow_number = i
                        break
...
vender_fat = ["",31,32,33,34,35,36]
vender_carl = ["",31,32,33,34,35,36]
# endregion
#초기화면
