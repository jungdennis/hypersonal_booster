    ...
    if count == 2:
        ...
        if before_or_after == 0:
            ...
            if check == 1:
                ...
                # endregion
                rate = float(required_gram / amount)
                powtext = "\n제품 : %s\n브랜드: %s\n주요 성분: %s\n용량 : %.1fg\n" \
                          "상세성분\n1회분 %dg당 %dkcal 단백질 %dg 탄수화물 %dg 지방%dg\n" \
                          "총 %.1fkcal 단백질 %.1fg 탄수화물 %.1fg 지방%.1fg" % (name, brand, sort, required_gram,
            ...
            # 자판기 안에 없음 비슷한거 찾음
                ...
                rate = float(changed_gram / amount)
                powtext = "\n제품 : %s\n브랜드: %s\n주요 성분: %s\n용량 : %.1fg\n" \
                          "상세성분\n1회분 %dg당 %dkcal 단백질 %dg 탄수화물 %dg 지방%dg\n" \
                          "총 %.1fkcal 단백질 %.1fg 탄수화물 %.1fg 지방%.1fg" % (name, brand, sort, changed_gram,
